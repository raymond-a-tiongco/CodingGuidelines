本コーディング規約は、フューチャー株式会社が無償で提供しているコーディング規約をADMATRIX向けにカスタマイズしたものです。  
...
* アクセス修飾子の宣言は適切な権限で行うこと（※ 制限の弱さの順に記載）
  1. `public`
  1. `protected`
  1. アクセス修飾子なし(パッケージ`private`)
  1. `private`
...
* Javadoc を記述する
* Javadoc コメントには、少なくとも`@author`、`@param`、`@return`と`@throws`を記述する
  * `@Override`のあるメソッドでは、上記に加えて`{@Inherit}`を記述する
  * 編集したファイルに編集者の`@author`がない場合、下に追加する
* Javadoc　クラスヘッダコメントのフォーマットは以下の通り  
     ...
     *    ・似たようなメソッドとの使い分け基準
     * @param form {パラメータの説明。}
     * @param response {パラメータの説明}
     * @return {戻り値の説明}
     * @throws IOException {例外の発生条件}
     */
    ```
...
* 原則としてstaticインポートしない
  * テストケースにおけるJUnitのstaticインポートは推奨する
  * フレームワークとしてはstaticインポートは推奨しない
...
* 配列宣言は「`型名[]`」にする  
    ...
    ```
* できるだけローカル変数を利用する  
    ローカル変数で事足りるものをインスタンス変数として利用するなど、必要のないインスタンス変数を定義すると、パフォーマンスや可読性の低下の大きな要因となる上、マルチスレッドを意識した際に不整合がおきる可能性(=スレッドセーフではない)があるので、インスタンス変数は必要性を充分に考慮してから使用すること。  
* 定数は`final`で宣言する  
...
* 例外
  * HadoopのMapper、Reducerはインナークラスを推奨する
  * データクラス（Bean, DTO, VO, Entity）
    * データクラスも原則として利用しない方が良いが、既存実装に色濃く残っているため許容する。
    * ただし、同クラス内からのアクセスのみとし、外部からのアクセスは許容しない。
...
* オブジェクト同士は`equals()`メソッドで比較する  
    ...
    定数と比較する場合、左辺に定数を書くこと

    良い例:  

    ```java
    private static final String S_1 = "text";
    ...
    String s2 = "text";
    if (S_1.equals(s2)) {
        //・・・
...
* 日付の文字列のフォーマットには、`FastDateFormat`または`DateTimeFormatter`を使う
* `SimpleDateFormat`はスレッドセーフではないのでフィールドには使用しないこと。

## 三項演算子
* 入れ子の三項演算子の利用は禁止  
    可読性が悪くなるので三項演算子を入れ子で行うのは禁止。  
    [参考](https://qiita.com/ikemo/items/f81f08055bb873b4d648)

## コレクション

* FastUtilを利用すること
* FastUtilを使用できない場合、次のことに気をつけること
...
* `ObjectOutputStream` は機能上、書き込まれるすべてのオブジェクトの参照がキャッシュ上に保持されるので、 `reset()` を利用する
...
# org.apache.commonsが提供するユーティリティの利用
